    ...
    describe('Solver.getDetailedPaths', () => {
        ...
        it('direction vectors are normalized', () => {
            ...
            for (const path of detailedPaths) {
                for (const reflection of path.reflections) {
                    ...
                    const inLen = Math.sqrt(reflection.incomingDirection[0] ** 2 + reflection.incomingDirection[1] ** 2);
                    expect(inLen).toBeCloseTo(1, 10);
                    // Check outgoing direction is normalized
                    const outLen = Math.sqrt(reflection.outgoingDirection[0] ** 2 + reflection.outgoingDirection[1] ** 2);
                    expect(outLen).toBeCloseTo(1, 10);
                    // Check wall normal is normalized
                    const normLen = Math.sqrt(reflection.wallNormal[0] ** 2 + reflection.wallNormal[1] ** 2);
        ...
        it('simplePath in detailed result matches original path', () => {
            ...
            for (const detailedPath of detailedPaths) {
                const matchingSimplePath = simplePaths.find(sp => JSON.stringify(sp) === JSON.stringify(detailedPath.simplePath));
        ...
        it('wall position correctly identifies hit location on wall', () => {
            ...
            const bottomReflection = detailedPaths.find(p => p.reflections.length === 1 && p.reflections[0].wallId === 0);
            if (bottomReflection) {
