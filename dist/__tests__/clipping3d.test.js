...
import { clipPolygonByPlane, clipPolygonByPlanes, quickRejectPolygon, clipRayByPlanes } from '../geometry/clipping3d';
describe('clipPolygonByPlane', () => {
    it('keeps polygon entirely in front', () => {
        const verts = [[0, 0, 5], [1, 0, 5], [1, 1, 5], [0, 1, 5]];
    ...
    it('clips spanning polygon', () => {
        const verts = [[0, 0, -1], [2, 0, -1], [2, 0, 1], [0, 0, 1]];
        const poly = Polygon3D.create(verts);
        const plane = Plane3D.fromNormalAndPoint([0, 0, 1], [0, 0, 0]);
        const result = clipPolygonByPlane(poly, plane);
        expect(result).not.toBeNull();
...
describe('clipPolygonByPlanes', () => {
    it('clips against multiple planes', () => {
        // Large polygon
        const verts = [[-10, -10, 0], [10, -10, 0], [10, 10, 0], [-10, 10, 0]];
        const poly = Polygon3D.create(verts);
...
describe('quickRejectPolygon', () => {
    it('returns true if polygon is entirely outside', () => {
        const verts = [[0, 0, 10], [1, 0, 10], [1, 1, 10], [0, 1, 10]];
        const poly = Polygon3D.create(verts);
    ...
    it('returns true if outside any single plane', () => {
        const verts = [[5, 5, 0], [6, 5, 0], [6, 6, 0], [5, 6, 0]];
        const poly = Polygon3D.create(verts);
        const planes = [
...
describe('clipRayByPlanes', () => {
    ...
    it('returns null if ray misses volume', () => {
        const origin = [0, 0, 10]; // Far from volume
        const direction = [1, 0, 0]; // Parallel to volume
        const planes = [
    ...
    it('handles ray starting inside volume', () => {
        const origin = [3, 0, 0]; // Inside volume
        const direction = [1, 0, 0];
        const planes = [
...
//# sourceMappingURL=clipping3d.test.js.map