...
let visualizationMode: 'paths' | 'sources' = 'paths';
...
const virtualSourceMap = new Map<THREE.Mesh, BeamVisualizationData>();

// Map from path mesh (tubes/spheres) to path data for click detection
const pathMeshMap = new Map<THREE.Mesh, ReflectionPath3D>();

// Currently selected virtual source
let selectedVirtualSource: THREE.Mesh | null = null;

// Currently selected path
let selectedPath: ReflectionPath3D | null = null;
 ...
 */
function beamHasValidPath(beam: BeamVisualizationData, paths: ReflectionPath3D[]): boolean {
 ...
 */
function drawVirtualSources(beams: BeamVisualizationData[]): void {
 ...
 */
function highlightSelectedPolygon(beam: BeamVisualizationData): void {
  clearHighlights();
 ...
 */
function highlightPath(path: ReflectionPath3D): void {
  clearHighlights();
  ...
  const listenerHighlight = new THREE.Mesh(
  ...
  if (!wasDragging) {
    ...
    if (!didMove) {
      ...
      if (visualizationMode === 'sources') {
        ...
        if (vsIntersects.length > 0) {
          const clickedMesh = vsIntersects[0].object as THREE.Mesh;
      ...
      if (visualizationMode === 'paths' && pathMeshMap.size > 0) {
        ...
        if (pathIntersects.length > 0) {
          const clickedMesh = pathIntersects[0].object as THREE.Mesh;
...
function toggleShowAllVirtualSources(): void {
