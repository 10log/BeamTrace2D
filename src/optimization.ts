...
import type { Point, ReflectionPath, PathPoint } from './beamtrace2d';
...
export interface SkipCircle {
  center: Point;    // Listener position when bucket failed
  radius: number;   // Minimum distance to any fail line in bucket
}
...
/**
 * Result of path validation with fail info
 */
interface ValidationResult {
  valid: boolean;
  path: ReflectionPath | null;
  failInfo: { type: FailLineType; line: Line2D; nodeDepth: number } | null;
  raycastCount: number;
}
...
class OptimizedBeamNode {
  ...
  constructor(
    public id: number,
    public parent: OptimizedBeamNode | null,
    public vs: Point,
    public windowP1?: Point,
    public windowP2?: Point
  ) {}
...
class BSPTree {
  ...
  private build(recursiveArray: BSPNode[]): BSPNode | null {
    if (recursiveArray.length === 0) return null;
 ...
 */
function getNodeDepth(node: OptimizedBeamNode): number {
  let depth = 0;
  let current: OptimizedBeamNode | null = node;
 ...
 */
function collectLeafNodes(node: OptimizedBeamNode, result: OptimizedBeamNode[]): void {
 ...
 */
function createBuckets(beamTree: OptimizedBeamTree, bucketSize: number = DEFAULT_BUCKET_SIZE): Bucket[] {
  const leafNodes: OptimizedBeamNode[] = [];
 ...
 */
function invalidateSkipCircle(bucket: Bucket): void {
...
export class OptimizedSolver {
  ...
  private readonly beamTree: OptimizedBeamTree;
  ...
  getPaths(listener: Listener): ReflectionPath[] {
    ...
    this.resetMetrics();
    const validPaths: ReflectionPath[] = [];
   ...
   */
  private findNonLeafPaths(listener: Listener, node: OptimizedBeamNode): ReflectionPath[] {
   ...
   */
  private validatePathOptimized(listenerPos: Point, leafNode: OptimizedBeamNode): ValidationResult {
    let raycastCount = 0;

    // Full path validation (same as basic solver)
    const pathPoints: PathPoint[] = [[listenerPos[0], listenerPos[1], null]];
    let currentPoint = listenerPos;
    let currentNode: OptimizedBeamNode | null = leafNode;
    let prevWallId = -1;
    ...
    while (currentNode && currentNode.id !== -1) {
      ...
      const intersection: Point = [int[0], int[1]];
  ...
  private traverseBeam(
    p0: Point,
    node: OptimizedBeamNode,
    prevNode: OptimizedBeamNode | null,
